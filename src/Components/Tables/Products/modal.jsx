    	...
    	if (mod === 'add') {
    		renderData = <div>
    			...
    			<Modal open={open} onClose={this.onCloseModal} center>
    				...
    				<form>
    					{data.map(element => ( 
    						<input key={element} onChange={onChange} name={element} placeholder={element} type={(element === 'date1' || element === 'date2' ? 'date' : 'text')} required/> 
    					))}
    				</form>
    		...
    		renderData = <div>
    			...
    			<Modal open={open} onClose={this.onCloseModal} center>
    				...
    				<form>
    					{data.map(element => ( 
    						<input key={element} onChange={onChange} defaultValue={item[element]} name={element} type={(element === 'date1' || element === 'date2' ? 'date' : 'text')} required/> 
    					))}
    				</form>
    				<p className="res">{respons}</p>
    				<Button color="warning" onClick={this.confirmed}> Confirmed </Button>
