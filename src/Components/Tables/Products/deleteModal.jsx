...
class DeleteModal extends Component {
	...
	componentDidMount() {
		...
		for(let i = this.props.item.quantity; i >= ONE; --i) { 
			quantityArr.push(i);
		}
    ...
    handleClick = (e) => {
    	if(e.target.id === 'toShop') {
    		this.setState({
    			senderValue: this.state.senders[ZERO].name + ',' + this.state.senders[ZERO].surname,
    			shopValue: this.state.shops[ZERO].name,
    			forceDelete: true,
    			status: this.props.item.status,
    			element: <FormGroup>
    				<Label for="senderSelect">Choose sender </Label>
    				<Input type="select" name="select" id="senderSelect" onChange={this.senderChange}>
    					{this.state.senders.map((item, index) => (
    						<option key={index} value={item.name + ',' + item.surname}>{item.name},{item.surname}</option>
    					))} 
    				</Input>
    				...
    				<Input type="select" name="select" id="quantity" onChange={this.quantityChange}>
    					{this.state.quantitySelect.map((item, index) => (
